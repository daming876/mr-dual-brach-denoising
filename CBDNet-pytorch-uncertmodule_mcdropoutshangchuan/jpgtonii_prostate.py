...
def convert_jpg_to_nii(input_root, output_root):
    # 遍历200个原始病例文件夹
    for case_dir in sorted(glob.glob(os.path.join(input_root, 'ProstateX-*'))):
        case_id = os.path.basename(case_dir).split('-')[-1]
        output_case_dir = os.path.join(output_root, f'ProstateX-{case_id}')
